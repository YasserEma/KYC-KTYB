    ...
    const fetchTenantBranding = async () => {
      try {
        ...
        if (effectiveSlug) {
          ...
          if (data && !error) {
            setTenantId(data.id);
            const brandData = data.brand as Record<string, any>;
            setTenantBrand({
              logo: brandData?.logo as string,
              primaryColor: brandData?.primaryColor as string || '#4f46e5', // Default to indigo if not set
              secondaryColor: brandData?.secondaryColor as string || '#818cf8',
              name: data.name
            });
          }
        }
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        {success ? (
          <div className="bg-green-50 border border-green-200 text-green-600 px-4 py-3 rounded text-center">
            ...
            <p className="mt-2">
              <Link href="/en/auth/login" className="text-green-700 underline">
                Return to login
              </Link>
            </p>
          </div>
